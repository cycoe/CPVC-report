...
\begin{figure}[!htb]
    \begin{center}
		\begin{tikzpicture}[node distance = 2cm]
			% nodes
			\node(pre)[pro]{配方设计};
			\node(smooth)[io, below of = pre, xshift = -2.5cm]{润滑剂};
			\node(stablizer)[io, below of = pre, xshift = 2.5cm]{热稳定剂};
			\node(sample)[pro, below of = smooth, xshift = 2.5cm]{制样};
			\node(exp)[pro, below of = sample]{性能测试};
			
			% arrows
			\draw[arrow](pre.west) -| (smooth.north);
			\draw[arrow](pre.east) -| (stablizer.north);
			\draw[arrow](smooth.south) |- (sample.west);
			\draw[arrow](stablizer.south) |- (sample.east);
			\draw[arrow](sample) -- (exp);
		\end{tikzpicture}
    \end{center}
    \caption{研究方案流程图}
\end{figure}
